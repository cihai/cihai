...
def cihai_full_options(
    unihan_full_options: "UnihanOptions", cihai_full_db_dsn: str
) -> "UntypedDict":
...
def cihai_quick_zip(
    ...
    cihai_sample_fixture_files: t.List[pathlib.Path],
) -> zipfile.ZipFile:
    _files = []
...
@pytest.fixture(scope="session")
def cihai_quick(
    cihai_quick_path: pathlib.Path, cihai_quick_options: "UntypedDict"
) -> "Cihai":
    """Setup a tiny portion of UNIHAN, return a UnihanOptions."""
...
@pytest.fixture(scope="session")
def cihai_user_path(
    cihai_home_path: pathlib.Path, cihai_home_user_name: str
) -> pathlib.Path:
